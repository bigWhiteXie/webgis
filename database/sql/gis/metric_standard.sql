...
COMMENT ON COLUMN metric_standard.evaluation_id IS '评价标准配置ID，关联到evaluation_standard_config表的id';
...
-- 创建索引
CREATE INDEX idx_metric_standard_evaluation_id ON metric_standard (evaluation_id);

-- 创建metric_code和quality_level的联合索引
CREATE INDEX metric_code_quality_level ON metric_standard (metric_code, quality_level);